  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {event.detail && (
          <p className="mt-1.5 text-[0.9rem] leading-relaxed text-[var(--text-1)]">
            {event.detail}
          </p>
        )}
...
export default function TimelinePage() {
  ...
  const weeks = [...new Set(TIMELINE.map((e) => weekOf(e.at)))].sort((a, b) => a - b);
  const attention = TIMELINE.filter(
    ...
    <div data-density="calm" className="h-full overflow-y-auto">
      <div className="mx-auto max-w-3xl px-4 py-6">
        <div className="flex items-baseline justify-between gap-4 mb-1">
          <h1 className="text-lg font-medium">Three weeks of care</h1>
          <span className="eyebrow">{TIMELINE.length} events</span>
        </div>
        <p className="text-[var(--text-1)] text-[0.9rem] mb-5">
          {attention > 0 ? (
            <>
              <strong className="text-[var(--amber)]">{attention} items need you.</strong> The rest
              was handled.
            </>
          ) : (
        ...
        {weeks.map((week) => (
          <section key={week} className="mb-5">
            ...
            <div className="panel overflow-hidden">
              {TIMELINE.filter((e) => weekOf(e.at) === week).map((event) => (
                <Event
                  key={event.id}
                  event={event}
                  conflictTarget={
